...
class MemDatasetRead(object):
    def __init__(self, loc,skip=[], **kwargs):
        self.loc = loc
...
#     if mf.xc == 'SCAN' and method == half_circle:
    ...
    scaling = emunu1/(emunu2 + 1e-10)


    vmunu2 = vmunu2*scaling
...
#     if not np.allclose(vmunu1,vmunu2,atol=1e-5):
...
#         plt.subplot(1,3,3)
#         plt.imshow(vmunu2 - vmunu1)
#         plt.colorbar()
#         plt.show()
        
    if print_stat:print({True: 'tau Potentials identical', False: 'tau Potentials not identical'}[np.allclose(vtmunu1,vtmunu2,atol=1e-5)])
#     if not np.allclose(vtmunu1,vtmunu2,atol=1e-5):
#         plt.figure(figsize=(16,8))
#         plt.subplot(1,3,1)
#         plt.imshow(vtmunu2)
#         plt.colorbar()
...
#         plt.subplot(1,3,3)
#         plt.imshow(vtmunu2 - vtmunu1)
#         plt.colorbar()
#         plt.show()
    if return_errors:
        
        return mf.grids.coords, mf.grids.weights, np.eye(len(vmunu2)), np.stack([scaling,scalingt]) , error
    else:
        
        return mf.grids.coords, mf.grids.weights, np.eye(len(vmunu2)),  np.stack([scaling,scalingt])